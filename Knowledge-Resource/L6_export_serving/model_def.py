...
class MiniTransformerLM(nn.Module):
    ...
    def forward(self, idx: torch.Tensor):
        B,T = idx.shape
        pos = torch.arrange(0,T,device = idx.device).unsqueeze(0)
...
class SimpleBPE:
    """
    Minimal loader to work with a tokenizer.json that contains:
    {"vocab": {"<pad>":0,"<bos>":1,"<eos>":2,"<unk>":3,"</w>":4, ...}}
    If your trained tokenizer is different, replace encode/decode accordingly.
    """
    ...
    def encode(self, text, add_special_bos_eos = False, max_len=256):
        ids = []
    ...
    def decode(self, ids):
        toks = [self.rev_vocab.get(int(i), "<unk>") for i in ids]
        words = []; cur = []
        for t in toks:
            if t == "</w>":
                words.append("".join(cur)); cur=[]
            elif t.startswith("<"): # Skip specials
                continue
            else:
                cur.append(t)
